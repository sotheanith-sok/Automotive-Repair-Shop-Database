...
create view Customer_v as select cFirstName, cLastName,'Prospective'as Category from Customer natural join Prospective
UNION select cFirstName, cLastName,'Premier'as Category from Customer natural join Premier 
UNION select cFirstName, cLastName,'Steady'as Category from Customer natural join Steady;
...
--Add start date
create view Mechanic_metor_v
select e1.eName as Mentor, e2.eName as Mentee, t.tsStartDate as TraningStartDate,t.tsEndDate as TraniningEndDate ,t.tsSkillTrained as TraniningSkill  from Mechanic m1 inner join Employee e1 on m1.eID=e1.eID right outer join TrainingSkill t
...
create view Premier_profits_v
select c.cFirstName as FirstName, c.cLastName as LastName, sum(m.miCost) as TotalCost, p.pAnnualFee from ItemWork i left outer join MaintainItem m on i.milID=m.miID left outer join MaintainOrder mo on i.moID=mo.moID 
left outer join Vehicle v on mo.vVin=v.vVin left outer join Customer c on c.cID=v.cID left outer join Premier p on c.cID=p.cID group by c.cID;
...
create view Prospective_resurrection_v
select cFirstName, cLastName from Customer c natural join PromotionContact p except (select cFirstName, cLastName from Customer c natural join PromotionContact p where p.pcDate<'2017-09-05'); 
-- Queries
